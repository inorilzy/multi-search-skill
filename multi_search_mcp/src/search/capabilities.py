...
PROVIDER_CAPABILITIES: dict[str, ProviderCapability] = {
    ...
    "reddit": ProviderCapability(
        name="reddit",
        public_name="reddit-old",
        kind=ProviderKind.SCRAPER,
        search=SearchCapability(can_search=False),
        scrape=ScrapeCapability(can_scrape=True, supports_markdown=True, supports_text=True, supports_site_policy=True),
        output=OutputCapability(returns_urls=False, returns_content=True, returns_platform_metadata=True, returns_engagement=True),
        operation=_op(AuthMode.NONE, rate_limit_sensitive=True),
        scrape_policy=ScrapePolicy.SCRAPER,
        best_for=("Fetching content from Reddit URLs returned by other search providers",),
        notes="Scraper backend only; Reddit search sources are intentionally disabled.",
    ),
