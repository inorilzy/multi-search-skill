...
| **route_meta（行为参数）** | route 除源之外的行为：默认抓几个、超时、返回数量、降级源、是否让 provider 直接返回正文（`want_content`）等。 | `search_runner.py` → `ROUTE_META` / `DEFAULT_ROUTE_META` |
| **want_content（正文内联）** | route_meta 字段。`True` 时让支持的 provider 在搜索响应里直接返回正文（Tavily `raw_content`、Exa `text`、Firecrawl `markdown`、Baidu `web_summary`）。只有 `fast` 路由开启。 | `search_runner.py` → `ROUTE_META["fast"]` |
