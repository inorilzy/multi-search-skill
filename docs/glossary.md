...
| **正文预览（Markdown）** | 搜索正文只存放在 `scrapes[].markdown`，按 `source_id` 对应候选，默认最多 6000 字符。`multi_search` 的 markdown/both 模式只在顶层 `markdown` 放正文，`scrapes` 保留元数据。`fetch_source` 的单页预览字段仍为 `body`；预览长度不截短缓存。 | `service.py` → `run_search_web` / `run_fetch_source` |
