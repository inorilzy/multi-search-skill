...
| **key 状态：transient_invalid** | 运行态暂时视为无效；连续收到 provider 的 invalid 分类失败时，前两次先冷却 15 分钟并在冷却期跳过，第三次才升级为 `invalid`。success、`rate_limit`、`quota_exhausted` 和其它非 invalid provider 结果都会清零 `invalid_strikes`。 | `state/key_state.py` → `TRANSIENT_INVALID` / `INVALID_COOLDOWN` / `INVALID_STRIKE_LIMIT` / `SQLiteKeyManager.record_result` |
...
`invalid_strikes` 只表示连续的 provider invalid 分类失败：每次 invalid 结果原子累加，并在同一转换中决定 `transient_invalid` 或永久 `invalid`；其它 provider 结果清零计数并按自身状态更新。抓取 target 错误在编排入口作为中立事件处理，既不增加也不清零计数，也不覆盖已有健康状态或冷却。时间字段到期后候选选择器会重新允许尝试，但数据库的 `status` 不会仅因时间流逝自动改写为 `active`。
