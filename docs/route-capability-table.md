...
| `body` | RRF 后获取的正文预览，默认最多 6000 字符 | 公共结果的 `body` | 阅读证据；完整取得的正文按策略缓存 |
...
`search_web` 与 `multi_search` 共用候选召回、两级 RRF 和正文获取流程。所有有效原始名次参与融合，不在源内或查询内截断 15 条；最终取前 15 条获取正文。成功时附 `body` / `body_truncated` / `body_backend`，失败保留原排名并附 `body_error`。`content` 始终保留摘要，允许留存的完整正文进入短期 ContentStore，可通过 `read_source` 读取预览以外的片段。
