...
Tavily / Exa / Firecrawl first and keeps Jina Reader as a no-key fallback.
...
默认 `--scrape-top 0`：只输出搜索结果列表，不额外输出全文抓取内容。传 `--scrape-top N` 后，搜索完成会把 Tavily / Exa / Firecrawl / Twitter 已返回的 `scraped_content` 放入抓取内容区，并从其余结果里按 PREFER 源和共识权重挑选候选 URL。候选 URL 最多 30 条，默认按 Tavily → Exa → Firecrawl 轮转抓取；如果这些 keyed reader 不可用或失败，再用 Jina Reader 兜底。Jina 无 key 也可用免费限流额度，`JINA_API_KEY` 只是可选增强。源自带全文不受候选 URL 数量限制。
...
python search.py "x" --jina-first 5                   # 强制前 5 条先走 Jina
python search.py "x" --no-jina                        # 不用 Jina 兜底
```
...
- 后端分配：候选 URL 默认在 tavily/exa/firecrawl 间 round-robin；Jina Reader 是最后兜底
- 单条 URL 失败时 `scrape_url_smart()` 自动 fallback：primary → remaining keyed readers → jina
...
flowchart TD
    ...
    SAFE -->|ok| FB{抓取成功?}
    FB -->|失败| FBC[fallback 链<br/>remaining keyed readers → jina]
    FBC --> SAFE
    FB -->|成功| SAN
