...
Candidate scrape cap: 30 URLs/run。源自带的网页正文会直接复用，不计入候选抓取请求。默认额外抓取用 Jina Reader / Exa Contents / Tavily Extract。
...
flowchart TD
    ...
    FAN[daemon worker threads + Queue<br/>deadline = --timeout / 60s<br/>到点返回已有结果 · 迟到源记 timeout]:::infra
    ...
    M[deduplicate<br/>归一化 URL · 合并 also_from<br/>共识权重打分 · 保留更长字段]:::infra

    %% ========== Scrape 决策 ==========
    M --> SCRQ{--scrape-top > 0?}
    SCRQ -->|否 / --no-scrape| OUT
    SCRQ -->|是| ALLOC[scrape 分配器<br/>优先 B/PREFER 源 · 再按共识权重<br/>每源上限默认 6 · 候选 URL 上限 30]

    ALLOC --> RR[Jina Reader /<br/>Exa Contents / Tavily Extract<br/>round-robin · key pool offset per URL]:::scrape
    ...
    %% ========== 渲染层 ==========
    OUT[format_results + format_scrapes<br/>诊断信息 · 共识列表 ·<br/>verbose 展示 AI Answer/snippet<br/>已 scrub 的错误信息]:::infra
