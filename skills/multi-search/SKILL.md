...
2. Inspect `results[].content` and the matching `scrapes[].markdown`, joined by
   `source_id`. Legacy `multi_search` may render bodies once in top-level
   `markdown`, with source references in each section. A
