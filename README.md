...
当前默认行为：`search_web` 使用 `default`（`web` 的兼容别名），全部有效候选参与 RRF，最终取前 15 条并获取正文。`results[].content` 保留摘要，正文只在 `scrapes[].markdown` 返回，按 `source_id` 关联，默认每篇最多 6000 字符；允许留存时完整取得的正文进入缓存，可用 `read_source` 读取预览以外的片段。旧 `scrape_top` / `scrape_per_source` 参数不再裁剪最终正文列表。
...
- `scrapes[]`：按相同 RRF 顺序返回统一抓取结果，正文只在 `markdown` 字段出现，默认每篇最多 6000 字符。
